...
async function loadPhoneToday(page = 1) {
  currentPage = page;
  ...
  const colspan = document.querySelector('#phone-table thead tr').children.length;
  let lastDate = '';
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (tEl) tEl.addEventListener('input', () => loadPhoneToday(1));
  loadPhoneToday();
});
